...
class Lapackb implements Lapack
{
    ...
    private function transpose__OLD(
     ...
     */
    private function get_identity(int $size, int $dtype, string $type) : FFI\CData
    {
    ...
    ) : void
    {
        ...
        $identity_n = $this->get_identity($n, $dtype, $type);
    ...
    ) : void
    {
        ...
        $identity_m = $this->get_identity($m, $dtype, $type);
    ...
    ) : void
    {
        ...
        $info = $info_p[0];
        if ($info != 0) { throw new RuntimeException("gesvd_ workspace query failed. error=$info", $info); }
        ...
        if ($info < 0) { throw new RuntimeException("gesvd_ parameter error. argument ".(-$info)." had an illegal value.", $info); }
        if ($info > 0) { /* Handle convergence failure if needed, e.g., log a warning */
